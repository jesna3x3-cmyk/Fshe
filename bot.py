...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                link = await self.create_chat_invite_link(chat_id=FORCE_SUB_CHANNEL, creates_join_request=True)
                self.invitelink = link.invite_link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot Can't Create Join Request Invite Link From Force Sub Channel !")
                self.LOGGER(__name__).warning(f"Please Double Check The FORCE_SUB_CHANNEL Value And Make Sure Bot Is Admin In Channel With Invite Users Via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_CHANNEL}")
                self.LOGGER(__name__).info("\nBot Stopped. https://t.me/MadflixBots_Support For Support")
                sys.exit()
        if FORCE_SUB_CHANNEL2:
            try:
                link = await self.create_chat_invite_link(chat_id=FORCE_SUB_CHANNEL2, creates_join_request=True)
        ...
        if FORCE_SUB_CHANNEL3:
            try:
                link = await self.create_chat_invite_link(chat_id=FORCE_SUB_CHANNEL3, creates_join_request=True)
        ...
        if FORCE_SUB_CHANNEL4:
            try:
                link = await self.create_chat_invite_link(chat_id=FORCE_SUB_CHANNEL4, creates_join_request=True)
                self.invitelink4 = link.invite_link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot Can't Create Join Request Invite Link From Force Sub Channel !")
